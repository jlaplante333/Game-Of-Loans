...
my-agent-directory/
├── src/
│   ├── /game-of-loans              # Agent configurations
├── apps/
...
- `src/game-of-loans/.env`: Inkeep CLI environment variables
...
Your graphs are defined in `src/game-of-loans/weather.graph.ts`. The default setup includes:

- **Weather Graph**: A graph that can forecast the weather in a given location.

Your inkeep configuration is defined in `src/game-of-loans/inkeep.config.ts`. The inkeep configuration is used to configure defaults for the inkeep CLI. The configuration includes:
...
1. Edit `src/game-of-loans/weather.graph.ts`
...
- Ensure you are runnning commands from `cd src/game-of-loans`.
- Validate the `inkeep.config.ts` file has the correct api urls.
- Validate that the `.env` file in `src/game-of-loans` has the correct `DB_FILE_NAME`.
